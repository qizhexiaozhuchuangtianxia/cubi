...
module.exports = React.createClass({
    ...
    componentDidMount: function() {
        //订阅 更新dirid和dirname
        this.app['APP-DASHBOARD-COPY-LIST-HANDLE'] = App.on('APP-DASHBOARD-COPY-LIST-HANDLE', this._modifyDirIdAndDirName);
        //弹出层设置顶部的返回方法
        this.app['APP-DASHBOARD-SET-DIRID-DIRNAME-COPY'] = App.on('APP-DASHBOARD-SET-DIRID-DIRNAME-COPY', this._setIdNameHandle);
        //每次点击设置一个存储用户id的数组
        this.app['APP-DASHBOARD-SET-USER-ID-COPY-HANDLE'] = App.on('APP-DASHBOARD-SET-USER-ID-COPY-HANDLE', this._setCanCopyArrHandle);
    },
    ...
    _setCanCopyArrHandle:function(obj){
        this.setState(function(previousState,currentProps){
            previousState.canCopyArr.push(obj.createUser);
    ...
    _openCreateDirHandle:function(){//弹出层新建目录的方法
        if(this.state.canCopyArr[this.state.canCopyArr.length-1]){
            if(this.state.canCopyArr[this.state.canCopyArr.length-1]){
                App.emit('APP-DASHBOARD-OPEN-CREATE-DIRECTIVE-DIALOG-HANDLE',{target:'COPY'});
            }else{
    ...
    _yesDialogHandle:function(){//弹出层 确定按钮的方法v
        ...
        if(this.state.canCopyArr[this.state.canCopyArr.length-1]){
            if(this.state.canCopyArr[this.state.canCopyArr.length-1]){
                DashboardStore.copyTargetHandle(targetId,dataObj).then(function(data){
                    if(data.success){
                        App.emit('APP-MESSAGE-OPEN',{
