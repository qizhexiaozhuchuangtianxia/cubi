...
module.exports = React.createClass({
	...
	getInitialState: function() {
		return {
			...
			canRemoveArr:[],
			seletedArr:[],//当前选中的数组
			edit:[],//存储编辑权限 移动 复制 新建 按钮操作
		}
	},
	componentDidMount: function() {
		//订阅 更新dirid和dirname
		this.app['APP-DASHBOARD-REMOVE-HANDLE'] = App.on('APP-DASHBOARD-REMOVE-HANDLE', this._modifyDirIdAndDirName);
		//弹出层设置顶部的返回方法
		this.app['APP-DASHBOARD-SET-DIRID-DIRNAME-REMOVE'] = App.on('APP-DASHBOARD-SET-DIRID-DIRNAME-REMOVE', this._setIdNameHandle);
		//每次点击设置一个存储用户id的数组
		this.app['APP-DASHBOARD-SET-USER-ID-REMOVE-HANDLE'] = App.on('APP-DASHBOARD-SET-USER-ID-REMOVE-HANDLE', this._setCanRemoveArrHandle);
	},
  ...
  //           	content:"您暂时没有权限移动当前文件夹下的文件夹和驾驶舱"
  //           });
		// }
	},
	...
	_removeDialogHandle:function(){//移动按钮提示框的方法
		...
		var actions = [
					...
					<PublicButton
					    style={{color:"rgba(0,229,255,255)"}}
							...
							onClick={this._openCreateDirHandle}	/>
	    ];
	    if(this.state.dirId == null){
	    	return  <Dialog
	    	            ...
	    	            onRequestClose={this._cancelDialogHandle}>
	    	            	...
	    	            	<RemoveListComponent seletedArr={this.state.seletedArr} dirId={this.state.dirId} loaded={this.state.loaded}
	...
	_openCreateDirHandle:function(){//弹出层新建目录的方法
		if(this.state.canRemoveArr[this.state.canRemoveArr.length-1]){
		    if(this.state.canRemoveArr[this.state.canRemoveArr.length-1]){
		        App.emit('APP-DASHBOARD-OPEN-CREATE-DIRECTIVE-DIALOG-HANDLE',{target:'REMOVE'});
		    }else{
		        App.emit('APP-MESSAGE-OPEN',{
	...
	_yesDialogHandle:function(){//弹出层 确定按钮的方法v
		...
		if(this.state.canRemoveArr[this.state.canRemoveArr.length-1]){
			if(this.state.canRemoveArr[this.state.canRemoveArr.length-1]){
				DashboardStore.removeTargetHandle(targetId,dataObj).then(function(data){
					if(data.success){
						App.emit('APP-DASHBOARD-REFRESH-LIST-HANDLE');
						App.emit('APP-MESSAGE-OPEN',{
