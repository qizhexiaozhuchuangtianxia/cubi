...
module.exports = React.createClass({
	...
	componentDidMount:function(){
		//订阅 添加文件夹之后刷新列表页
		this.app['APP-DASHBOARD-REMOVE-REFRESH-LIST-HANDLE'] = App.on('APP-DASHBOARD-REMOVE-REFRESH-LIST-HANDLE', this._removeRefreshListHandle);
		this._scrollBrar();
	},
	...
	_getPathDataHandle:function(resourceId) {//获取面包屑导航的方法
		...
		if(resourceId){
			DashboardStore.getPathDataHandle(resourceId).then(function(data){//获取面包屑的接口
				...
				if(dataLen>0){
					App.emit('APP-DASHBOARD-SET-DIRID-DIRNAME-REMOVE',data.dataObject[arrIndex])
				}else{
					App.emit('APP-DASHBOARD-SET-DIRID-DIRNAME-REMOVE',goBackObj)
				}
	...
	_onTouchTapHandle:function(item){//_onTouchTapHandle点击列表执行的方法
		if(item.type == 'DIRECTORY'){
			...
			//执行修改方法订阅在RemoveButton.js
			App.emit('APP-DASHBOARD-REMOVE-HANDLE',{
				'dirId':item.id,
				'dirName':item.name,
			})
			App.emit('APP-DASHBOARD-SET-USER-ID-REMOVE-HANDLE',{
