...
module.exports = React.createClass({
	...
	componentWillMount:function(){
		this._getTeamArrAndSetSaveDataArr(this.props.dashboardId);//获取页面需要的数据和拼接保存data的数据		
	},
	componentWillReceiveProps:function(nextProps){//点击本页面进入本页面的时候触发
		this._getTeamArrAndSetSaveDataArr(nextProps.dashboardId);	
		this._scrollHandle();
	},
	...
	_addGroupHandle:function(){
		App.emit('APP-DASHBOARD-REFRESH-OPEN-ADD-GROUP-HANDLE');
	},
	_getTeamArrAndSetSaveDataArr:function(dashboardId){//获取渲染页面的数据和设置保存数据的结构的方法
		var _this = this;
		AuthorityStore.getAuthorListData(dashboardId).then(function(data){//获取页面渲染需要的数据
			/*拼接保存时候需要的data对象*/
			var arrData = [];
			if(data.length>0){
					...
					<ListItem className="listItemBox" onClick={(evt)=>this._listItemClick(evt,key)}>
						<div className="listItem">
							...
							<div className="textBox">
								<div className="name">{item.authorityName}</div>
	...
	_listItemClickHandle:function(evt,item){//点击权限修改保存参数的对象
		evt.stopPropagation();
		var _this = this;
		//this._addListItemStyle(item.authorityListIndex);
		this.setState(function(state){
