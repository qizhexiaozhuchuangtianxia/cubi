...
class FilterSearchComponent extends Component {
	...
	_onKeyUpHandle(event,keyword){//onKeyUp搜索执行的方法
        ...
        this.timer = setTimeout(function(){
            App.emit('APP-DASHBOARD-REPORT-SEARCH-FILTER-VALEU',_this.props.seletedData,1,inputValue);       
        },200);
    ...
    _closeKeyUpHandle(evt){
        evt.stopPropagation();
        var inputValue = '';
        App.emit('APP-DASHBOARD-REPORT-SEARCH-FILTER-VALEU',this.props.seletedData,1,inputValue);
        this.setState({
            openSearchComponent:false
        })
    }
