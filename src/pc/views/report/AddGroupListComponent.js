...
module.exports = React.createClass({
	...
	componentWillMount:function(){
		this._getAddGroupListHandle(this.props.reportId);
	},
	componentWillReceiveProps:function(nextProps){//点击本页面进入本页面的时候触发
		this._getAddGroupListHandle(nextProps.reportId);		
	},
	...
	_getAddGroupListHandle:function(reportId){//获取添加分组的所有列表数据的方法
		var _this = this;
		var parentId = this.state.parentId||'';//以后有树形结构可以修改这个state
		AuthorityStore.getAuthorityTreeHandle(reportId,parentId).then(function(resultData){
			if(_this.isMounted()){
				_this.setState({
	...
	_addTeamHandle:function(evt,item){//添加分组的方法
		var _this = this;
		AuthorityStore.getAuthorListData(this.props.reportId).then(function(data){//获取页面渲染需要的数据
