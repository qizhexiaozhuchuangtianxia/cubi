...
module.exports = React.createClass({
	...
	componentDidMount:function(){
		//订阅 添加文件夹之后刷新列表页
		this.app['APP-REPORT-COPY-REFRESH-LIST-HANDLE'] = App.on('APP-REPORT-COPY-REFRESH-LIST-HANDLE', this._removeRefreshListHandle);
		this._scrollBrar();
	},
	...
	_getPathDataHandle:function(resourceId) {//获取面包屑导航的方法
		...
		if(resourceId){
			ReportStore.getPathDataHandle(resourceId).then(function(data){//获取面包屑的接口
				...
				if(dataLen>0){
					App.emit('APP-REPORT-SET-DIRID-DIRNAME-COPY',data.dataObject[arrIndex])
				}else{
					App.emit('APP-REPORT-SET-DIRID-DIRNAME-COPY',goBackObj)
				}
	...
	_onTouchTapHandle:function(item){//_onTouchTapHandle点击列表执行的方法
		if(item.category == 'DIRECTORY'){
			...
			App.emit('APP-REPORT-COPY-LIST-HANDLE',{
				'dirId':item.id,
				'dirName':item.name,
				'canEdited':item.canEdited 
			})
			App.emit('APP-REPORT-SET-CAN-COPY-ARR-HANDLE',{
