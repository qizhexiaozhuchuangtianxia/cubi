...
var ListRightShowComponent = require('./ListRightShowComponent');
...
module.exports = React.createClass({
	...
	getInitialState: function() {
		return {
			...
			userRoot:{},
			resourceId:'',
			currentCanEdited:true,
			canRemoveId:null
		}
	},
	componentWillMount:function(){//驾驶舱列表页面
		this.setState({
			currentCanEdited:this.props.currentCanEdited,
			canRemoveId:this.props.dirId
		})
		this._getReportListHandle(this.props.dirId,this.props.field,this.props.method);//获取数据的方法
		this._getPathDataHandle(this.props.dirId);//获取面包屑导航的方法
		this._getRootHandle('Report');//获取当前用户对当前模块的权限的方法
	},
	componentWillReceiveProps:function(nextProps){//点击本页面进入本页面的时候触发
		this.setState({
			...
			currentCanEdited:nextProps.currentCanEdited,
			canRemoveId:nextProps.dirId
		})
		this._getReportListHandle(nextProps.dirId,nextProps.field,nextProps.method);//获取数据的方法
		this._getPathDataHandle(nextProps.dirId);//获取面包屑导航的方法
		this._getRootHandle('Report');//获取当前用户对当前模块的权限的方法
	},
	componentDidMount: function() {
		//订阅 添加文件夹和文件刷新当前页面
		this.app['APP-REPORT-REFRESH-LIST-HANDLE'] = App.on('APP-REPORT-REFRESH-LIST-HANDLE', this._refreshDashaboardHandle);
		this._scrollBrar();
	},
	componentDidUpdate: function() {
		...
		if(this.state.userRoot){
			if(true){//当前用户是否可以在我的分析列表页面看到 配置权限按钮
				authorityComponent = <AuthorityButton resourceId={this.state.resourceId} authority={this.state.authority} checkLenProps={this.state.checkLen}/>;
			}
    ...
    _deleteOneHandleSubmit:function(){//删除方法只能删除一个 下一期会去掉//确定删除
    	...
    	ReportStore.deleteReportList(deleteId).then(function(resultData){
    		if(resultData.success){
    			_this._getReportListHandle(_this.props.dirId,_this.props.field,_this.props.method);
    			_this.setState({
		    		openDelete:false
		    	})
    		}else{
    ...
    _deleteDialogOpen:function(){//打开删除弹窗
    	...
    	for(var i=0;i<this.state.listData.length;i++){
    		if(this.state.listData[i].listCheckBoxBool){
    			if(this.state.listData[i].authorityDelete && this.state.currentCanEdited){
    				canDeleted = true;
    			}else{
    				canDeleted = false;
    			}
    			if(this.state.listData[i].category != 'DIRECTORY'){
    				deletedTipText = '您暂时没有权限删除当前选中的分析'
    			}
    		}
    	}
    	if(canDeleted){
    ...
    _reNameOneHandleSubmit:function(evt,reName){//重命名提交方法
    	var _this = this;
    	...
    	if(this.state.errorTextState !=''){
			...
			ReportStore.reNameReportList(reNameId,reNameName).then(function(resultData){
				if(resultData.success){
	    			_this.setState({
			    		openReName:false,
			    		thisVal:'',
    ...
    _reNameDialogOpen:function(){//重命名打开弹出框
    	...
    	for(var i=0;i<this.state.listData.length;i++){
    		if(this.state.listData[i].listCheckBoxBool){
    			...
    			if(this.state.listData[i].category != 'DIRECTORY'){
    				renameTipText = '您暂时没有权限重命名当前选中的分析'
    			}
    		}
    	}
    	if(canReName){
    ...
    _getReNameInputVal:function(evt){//
    	evt.stopPropagation();
	...
	_refreshDashaboardHandle:function(){//订阅的事件 点击刷新了驾驶舱的方法 添加一个文件就会刷新本页面
		this._getReportListHandle(this.props.dirId,this.props.field,this.props.method);
	},
	_getReportListHandle:function(dirId,field,method){//获取列表数据的方法
		var _this = this;
		var dataObj = {
			"queryParameters": {
				"sortParameters": [{
		...
		if(dirId){
			ReportStore.getReportData(dirId).then(function(data){
				_this.setState({
					currentCanEdited:data.dataObject.authorityEdit
				})
			});
		}
	},
	_getRootHandle:function(modelStr){//获取用户对当前模块的权限的方法
		var _this = this;
		ReportStore.getRootDataHandle(modelStr).then(function(data){//获取用户权限的接口
			if(data.success){
				_this.setState({
	...
	_getPathDataHandle:function(resourceId) {//获取面包屑导航的方法
		var _this = this;
		var currentName = [{name:'我的分析',id:null,authorityEdit:true}];
		if(resourceId){
	...
	_clickListHandle:function(evt,itemObj,iconClassName){//列表点击事件方法 如果是文件夹 进行列表 如果文件 右侧弹出
		evt.stopPropagation();

		if(itemObj.category == "DIRECTORY"){
			this.context.router.push({pathname:'/report/list',query:{
					'dirId':itemObj.id,
					'currentCanEdited':itemObj.authorityEdit
				}
			});
		}else{
			ReportStore.tableCustomArr=[];
			App.emit('APP-DRAWER-RIGHT', <ListRightShowComponent
				fileId={itemObj.id}
				...
				authority={itemObj.authority}
				pathState={this.state.pathState}
				pathname={this.props.pathname}
				userRoot={this.state.userRoot}/>,392);
		}
	},
	_checkBoxHandle:function(evt,index){//点击单个选择的效果方法
		...
		this.setState(function(previousState,currentProps){
			...
			previousState.selectedAuthorityEdit=previousState.listData[index].authorityEdit;//当前选中的是否可编辑
	...
	render: function() {
		var addComponent = <AddComponent
							...
							dirName={this.props.dirName}
							listData = {this.state.listData}/>;
	 	if(!this.state.currentCanEdited){
 			addComponent = null;
	 	}
			...
			if(this.state.listData.length>0){
				...
				return (
					<div className="listComponentBox">
						...
						<div className="listComponentBoxDown">
							<div className="cubi-error getReportNullError">当前数据为空</div>
						</div>
					</div>
				)
			}
